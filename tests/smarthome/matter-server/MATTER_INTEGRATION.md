...
### 6.3 Persist the VLAN sub-interfaces

The sub-interfaces created in §6.1 are lost on reboot. In this repo the Ansible
`vlan-trunk` role now auto-detects whether the node is using `ifupdown` or
`systemd-networkd` and writes the matching persistent configuration.

If you want to configure the node manually, `systemd-networkd` still works like this:
...
The `node_setup.yml` playbook now includes the `vlan-trunk` role. You can also run
`ansible/playbooks/vlan_trunk_setup.yml` directly.

Any host with `vlan_trunk_interfaces` defined will get the matching persistent VLAN
configuration automatically:

- `ifupdown` hosts get files in `/etc/network/interfaces.d/`
- `systemd-networkd` hosts get `.netdev` / `.network` files in `/etc/systemd/network/`
